...
ELIXIP_PORT=${BAUDOT_ELIXIP_SIP_PORT:-5292}
JAIN_PORT=5290
SCENARIO="$ROOT/interop/elixip/interop004-elixip-to-jain-positive-handoff.exs"
EVIDENCE=${BAUDOT_EVIDENCE_DIR:-$ROOT/target/evidence-external}
CORRELATION=elixip-to-jain-positive-handoff-v1
OUT="$EVIDENCE/BAUDOT-INTEROP-004/$CORRELATION"
RUN="$OUT/elixip-to-jain-positive-handoff"
JAIN_PID=""
ELIXIP_PID=""
...
for bin in python3 java mvn git sha256sum ss timeout; do
...
media.stimulus.owner=Baudot-controlled-provider-b
...
BAUDOT_EVIDENCE_DIR="$EVIDENCE" \
  java -cp "$CP" org.mcc0nnell.baudot.harness.ElixipToJainReferPositiveHandoffProbe \
...
for _ in $(seq 1 100); do
  if ! kill -0 "$JAIN_PID" 2>/dev/null; then
    echo "JAIN reverse positive transfer processor exited before binding UDP/$JAIN_PORT" >&2
...
[[ "$jain_ready" == 1 ]] || { echo "JAIN reverse positive processor did not bind UDP/$JAIN_PORT" >&2; exit 3; }
...
if [[ "$ELIXIP_STATUS" -ne 0 || "$JAIN_STATUS" -ne 0 ]]; then
  echo "reverse positive interop execution failed: elixip=$ELIXIP_STATUS jain=$JAIN_STATUS" >&2
...
python3 -m scripts.validate_elixip_to_jain_refer_positive_handoff \
  --run-dir "$RUN" \
...
(
  ...
  required=(
    ...
    jain.stderr.log
    jain-listener.txt
    elixip-to-jain-positive-handoff/manifest.sha256
    elixip-to-jain-positive-handoff/result.properties
    elixip-to-jain-positive-handoff/terminal-result.json
    elixip-to-jain-positive-handoff/terminal.manifest.sha256
    elixip-to-jain-positive-handoff/original-invite.request.sip
    elixip-to-jain-positive-handoff/original-ack.request.sip
    elixip-to-jain-positive-handoff/refer.request.sip
    elixip-to-jain-positive-handoff/refer-202.response.sip
    elixip-to-jain-positive-handoff/replacement-invite.request.sip
    elixip-to-jain-positive-handoff/replacement-response-200.sip
    elixip-to-jain-positive-handoff/replacement-ack.request.sip
    elixip-to-jain-positive-handoff/rtt-datagram-sent.bin
    elixip-to-jain-positive-handoff/rtt-datagram-received.bin
    elixip-to-jain-positive-handoff/notify-200.request.sip
    elixip-to-jain-positive-handoff/old-leg-bye-sent.request.sip
    elixip-to-jain-positive-handoff/old-leg-bye-200.response.sip
  )
