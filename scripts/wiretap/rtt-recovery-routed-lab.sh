...
CNS=${BAUDOT_CALLER_NS:-bdt-rtt-recovery-caller}
SNS=${BAUDOT_CALLEE_NS:-bdt-rtt-recovery-callee}
WORK=${BAUDOT_WIRETAP_DIR:-$ROOT/target/wiretap-routed/004}
...
cleanup() {
  ...
  ip link del bdt-rtr-ul-h 2>/dev/null
  ip link del bdt-rtr-sig-h 2>/dev/null
  ip link del bdt-rtr-med-h 2>/dev/null
  rm -rf "$WORK"
...
ip link add bdt-rtr-ul-h type veth peer name bdt-rtr-ul-c
ip link set bdt-rtr-ul-c netns "$CNS"
ip addr add "$UL_HOST/24" dev bdt-rtr-ul-h; ip link set bdt-rtr-ul-h up
ip -n "$CNS" addr add "$UL_CLIENT/24" dev bdt-rtr-ul-c; ip -n "$CNS" link set bdt-rtr-ul-c up

ip link add bdt-rtr-sig-h type veth peer name bdt-rtr-sig-s
ip link set bdt-rtr-sig-s netns "$SNS"
ip addr add "$SIG_HOST/24" dev bdt-rtr-sig-h; ip link set bdt-rtr-sig-h up
ip -n "$SNS" addr add "$SIP_SERVER/24" dev bdt-rtr-sig-s; ip -n "$SNS" link set bdt-rtr-sig-s up

ip link add bdt-rtr-med-h type veth peer name bdt-rtr-med-s
ip link set bdt-rtr-med-s netns "$SNS"
ip addr add "$MEDIA_HOST/24" dev bdt-rtr-med-h; ip link set bdt-rtr-med-h up
ip -n "$SNS" addr add "$MEDIA_SERVER/24" dev bdt-rtr-med-s; ip -n "$SNS" link set bdt-rtr-med-s up
...
rtt.emittedSequenceNumbers=0,2
rtt.omittedSequenceNumbers=1
