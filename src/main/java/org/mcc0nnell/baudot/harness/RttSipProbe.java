...
/** Routed RTT interoperability probe with independently validated packet semantics. */
public final class RttSipProbe {
    ...
    static final int SSRC = 0x42415544;
    ...
    private static int runCaller(Config config) throws Exception {
        try (EvidenceRecorder evidence = new EvidenceRecorder(
            ...
            evidence.event("scenario.started", Map.of(
                    "role", "caller", "scenario", config.scenarioId(), "correlation", config.correlationId(),
                    "modality", "rtt", "profile", config.profile().wireName()));
            endpoint.start();
            endpoint.sendInvite();
            boolean established = endpoint.awaitDialog(config.timeout());
            boolean sent = established && sendRttDatagrams(config, evidence);
            evidence.result(Map.of(
                    "correlation.id", config.correlationId(), "media.probe.sent", Boolean.toString(sent),
                    "role", "caller", "rtt.datagrams.expected", Integer.toString(config.profile().sentDatagramCount()),
                    "rtt.profile", config.profile().wireName(), "scenario.expectMedia", "true",
                    "scenario.id", config.scenarioId(), "signaling.dialog.established", Boolean.toString(established)));
    ...
    private static int runCallee(Config config) throws Exception {
        try (EvidenceRecorder evidence = new EvidenceRecorder(
            ...
            evidence.event("scenario.started", Map.of(
                    "role", "callee", "scenario", config.scenarioId(), "correlation", config.correlationId(),
                    "modality", "rtt", "profile", config.profile().wireName()));
            receiver.start();
            endpoint.start();
            ...
            evidence.result(Map.of(
                    "correlation.id", config.correlationId(), "media.probe.received", Boolean.toString(datagramsReceived),
                    "role", "callee", "rtt.datagrams.expected", Integer.toString(config.profile().receivedDatagramCount()),
                    "rtt.profile", config.profile().wireName(), "scenario.expectMedia", "true",
                    "scenario.id", config.scenarioId(), "signaling.ack.received", Boolean.toString(endpoint.ackReceived()),
        ...
        try {
            if (config.profile() == Profile.RECOVERY) {
                ...
                evidence.event("rtt.loss.injected", Map.of(
                        "injection", "controlled-source-omission", "emitted", "0,2", "omitted", "1",
                        "classification", "sender-controlled"));
            } else if (config.profile() == Profile.NETWORK_LOSS) {
                String expectation = "{\n"
                        + "  \"injection\": \"callee-network-ingress-drop\",\n"
                ...
                evidence.event("rtt.network.loss.expected", Map.of(
                        "injection", "callee-network-ingress-drop", "sent", "0,1,2",
                        "expectedReceived", "0,2", "drop", "1"));
            }
            try (DatagramSocket socket = new DatagramSocket(new InetSocketAddress(
                ...
                for (int index = 0; index < datagrams.length; index++) {
                    ...
                    DatagramPacket packet = new DatagramPacket(content, content.length,
                            InetAddress.getByName(config.mediaTargetIp()), config.mediaTargetPort());
                    socket.send(packet);
                    evidence.event("rtt.datagram.sent", Map.of(
                            "index", Integer.toString(index + 1), "target", config.mediaTargetIp() + ":" + config.mediaTargetPort(),
                            "bytes", Integer.toString(content.length), "classification", "unvalidated",
                            "profile", config.profile().wireName()));
                    if (index + 1 < datagrams.length) Thread.sleep(50L);
                }
            }
            return true;
        } catch (Exception e) {
    ...
    static byte[] normalPrimaryPacket() { return directPacket(1, 1000, true, 'H'); }
    static byte[] normalRedPacket() { return redPacket(2, 1300, 'H', 'i'); }
    static byte[] recoveryPriorPacket() { return directPacket(0, 700, true, 'A'); }
    static byte[] networkDroppedPrimaryPacket() { return directPacket(1, 1000, false, 'B'); }
    static byte[] recoveryRedPacket() { return redPacket(2, 1300, 'B', 'C'); }
    ...
    enum Role { CALLER, CALLEE }

    enum Profile {
        NORMAL("normal", 2, 2), RECOVERY("recovery", 2, 2), NETWORK_LOSS("network-loss", 3, 2);
        ...
        int sentDatagramCount() { return sentDatagramCount; }
        int receivedDatagramCount() { return receivedDatagramCount; }
    ...
    record Config(Role role, Profile profile, String scenarioId, String correlationId,
            String callerSipIp, int callerSipPort, String calleeSipBindIp, String calleeSipIp, int calleeSipPort,
            int mediaSourcePort, String mediaBindIp, int mediaBindPort, String mediaTargetIp, int mediaTargetPort,
            Duration timeout, Path evidenceRoot) {
        static Config fromEnvironment() {
            Role role = Role.valueOf(env("BAUDOT_ROLE", "caller").trim().toUpperCase());
            ...
            return new Config(role, profile, scenario, correlation, callerIp,
                    envInt("BAUDOT_CALLER_SIP_PORT", 5070), env("BAUDOT_CALLEE_SIP_BIND_IP", calleeIp), calleeIp,
                    envInt("BAUDOT_CALLEE_SIP_PORT", 5080), envInt("BAUDOT_MEDIA_SOURCE_PORT", 40001), mediaBindIp,
                    envInt("BAUDOT_MEDIA_BIND_PORT", 40000), env("BAUDOT_MEDIA_TARGET_IP", mediaBindIp),
                    envInt("BAUDOT_MEDIA_TARGET_PORT", 40000), Duration.ofMillis(envInt("BAUDOT_TIMEOUT_MS", 5000)),
        ...
        private static int envInt(String name, int fallback) { return Integer.parseInt(env(name, Integer.toString(fallback))); }
    }
    ...
    private static final class RttReceiver implements AutoCloseable {
        ...
        RttReceiver(Config config, EvidenceRecorder evidence) {
            this.config = config;
            this.evidence = evidence;
            this.received = new CountDownLatch(config.profile().receivedDatagramCount());
        }
        void start() throws IOException {
            socket = new DatagramSocket(new InetSocketAddress(InetAddress.getByName(config.mediaBindIp()), config.mediaBindPort()));
            socket.setSoTimeout(500);
            running.set(true);
            thread = new Thread(this::receiveLoop, "baudot-rtt-datagram-receiver");
            thread.setDaemon(true);
            thread.start();
            evidence.event("rtt.receiver.ready", Map.of("bind", config.mediaBindIp() + ":" + config.mediaBindPort(),
                    "classification", "opaque-udp", "profile", config.profile().wireName()));
        }
        private void receiveLoop() {
            byte[] buffer = new byte[2048];
            int index = 0;
            while (running.get() && index < config.profile().receivedDatagramCount()) {
                DatagramPacket packet = new DatagramPacket(buffer, buffer.length);
                try {
                    socket.receive(packet);
                    index++;
                    byte[] content = Arrays.copyOfRange(packet.getData(), packet.getOffset(), packet.getOffset() + packet.getLength());
                    evidence.writeBytes("rtt-datagram-" + index + "-received.bin", content);
                    evidence.event("rtt.datagram.received", Map.of("index", Integer.toString(index),
                            "source", packet.getAddress().getHostAddress() + ":" + packet.getPort(),
                            "bytes", Integer.toString(packet.getLength()), "classification", "unvalidated",
                            "profile", config.profile().wireName()));
                    received.countDown();
                } catch (SocketTimeoutException ignored) {
                } catch (IOException e) {
                    if (running.get()) evidence.event("rtt.receiver.error", Map.of("error", e.toString()));
                    return;
                }
            }
        }
        boolean await(Duration timeout) throws InterruptedException { return received.await(timeout.toMillis(), TimeUnit.MILLISECONDS); }
    ...
    private static final class SipEndpoint implements SipListener, AutoCloseable {
        ...
        private SipEndpoint(Config config, EvidenceRecorder evidence, boolean caller) throws Exception {
            ...
            properties.setProperty("javax.sip.STACK_NAME", "baudot-rtt-" + (caller ? "caller-" : "callee-") + config.correlationId().replace("-", ""));
        ...
        static SipEndpoint caller(Config c, EvidenceRecorder e) throws Exception { return new SipEndpoint(c, e, true); }
        static SipEndpoint callee(Config c, EvidenceRecorder e) throws Exception { return new SipEndpoint(c, e, false); }
        void start() throws Exception { stack.start(); evidence.event("sip.endpoint.ready", Map.of("role", caller ? "caller" : "callee", "transport", "udp")); }
        void sendInvite() throws Exception {
            if (!caller) throw new IllegalStateException("Only the caller can send INVITE");
            ...
            Address fromAddress = addressFactory.createAddress(sipUri("caller", config.callerSipIp(), config.callerSipPort()));
            FromHeader from = headerFactory.createFromHeader(fromAddress, randomTag());
            Address toAddress = addressFactory.createAddress(sipUri("callee", config.calleeSipIp(), config.calleeSipPort()));
            ToHeader to = headerFactory.createToHeader(toAddress, null);
            List<ViaHeader> vias = new ArrayList<>();
            ViaHeader via = headerFactory.createViaHeader(config.callerSipIp(), config.callerSipPort(), ListeningPoint.UDP, null);
            via.setRPort();
            vias.add(via);
            CallIdHeader callId = provider.getNewCallId();
            CSeqHeader cseq = headerFactory.createCSeqHeader(1L, Request.INVITE);
            MaxForwardsHeader maxForwards = headerFactory.createMaxForwardsHeader(70);
            Request invite = messageFactory.createRequest(requestUri, Request.INVITE, callId, cseq, from, to, vias, maxForwards);
            ...
            evidence.event("sip.invite.sent", Map.of("target", config.calleeSipIp() + ":" + config.calleeSipPort(),
                    "callId", callId.getCallId(), "rport", "requested", "modality", "rtt"));
            inviteTransaction.sendRequest();
        }
        boolean awaitDialog(Duration timeout) throws InterruptedException { return dialog.await(timeout.toMillis(), TimeUnit.MILLISECONDS); }
        boolean inviteReceived() { return inviteReceived.get(); }
        boolean ackReceived() { return ackReceived.get(); }
        @Override public void processRequest(RequestEvent event) {
            if (caller) return;
            Request request = event.getRequest();
            if (Request.ACK.equals(request.getMethod())) {
                ackReceived.set(true); dialog.countDown(); evidence.event("sip.ack.received", Map.of()); return;
            }
            if (!Request.INVITE.equals(request.getMethod())) return;
            try {
                ...
                evidence.event("sip.invite.received", Map.of("callId", callId == null ? "unknown" : callId.getCallId(), "modality", "rtt"));
                byte[] rawOffer = request.getRawContent();
                if (rawOffer == null || rawOffer.length == 0) throw new IllegalStateException("RTT INVITE did not contain SDP");
                evidence.writeBytes("offer.sdp", rawOffer);
                evidence.event("sdp.offer.captured", Map.of("bytes", Integer.toString(rawOffer.length), "classification", "unvalidated"));
                Response ok = messageFactory.createResponse(Response.OK, request);
                ToHeader to = (ToHeader) ok.getHeader(ToHeader.NAME);
                if (to.getTag() == null) to.setTag(randomTag());
                Address contactAddress = addressFactory.createAddress(sipUri("callee", config.calleeSipIp(), config.calleeSipPort()));
                ContactHeader contact = headerFactory.createContactHeader(contactAddress);
                ok.addHeader(contact);
                ok.setContent(answerSdp(config), headerFactory.createContentTypeHeader("application", "sdp"));
                transaction.sendResponse(ok);
                evidence.event("sip.200.sent", Map.of("status", "200", "modality", "rtt"));
            } catch (Exception e) { evidence.event("sip.request.error", Map.of("error", e.toString())); }
        }
        @Override public void processResponse(ResponseEvent event) {
            if (!caller) return;
            Response response = event.getResponse();
            CSeqHeader cseq = (CSeqHeader) response.getHeader(CSeqHeader.NAME);
            if (response.getStatusCode() != Response.OK || cseq == null || !Request.INVITE.equals(cseq.getMethod())) return;
            try {
                byte[] rawAnswer = response.getRawContent();
                if (rawAnswer == null || rawAnswer.length == 0) throw new IllegalStateException("RTT 200 OK did not contain SDP");
                evidence.writeBytes("answer.sdp", rawAnswer);
                evidence.event("sdp.answer.captured", Map.of("bytes", Integer.toString(rawAnswer.length), "classification", "unvalidated"));
                Dialog sipDialog = event.getDialog();
                if (sipDialog == null && inviteTransaction != null) sipDialog = inviteTransaction.getDialog();
                if (sipDialog == null) throw new IllegalStateException("200 OK arrived without a SIP dialog");
                Request ack = sipDialog.createAck(cseq.getSeqNumber());
                sipDialog.sendAck(ack);
                dialog.countDown();
                evidence.event("sip.dialog.established", Map.of("status", Integer.toString(response.getStatusCode()), "modality", "rtt"));
            } catch (Exception e) { evidence.event("sip.response.error", Map.of("error", e.toString())); }
        }
        @Override public void processTimeout(TimeoutEvent event) { evidence.event("sip.timeout", Map.of("server", Boolean.toString(event.isServerTransaction()))); }
        @Override public void processIOException(IOExceptionEvent event) { evidence.event("sip.io_error", Map.of("host", String.valueOf(event.getHost()), "port", Integer.toString(event.getPort()), "transport", String.valueOf(event.getTransport()))); }
        @Override public void processTransactionTerminated(TransactionTerminatedEvent event) { evidence.event("sip.transaction.terminated", Map.of("server", Boolean.toString(event.isServerTransaction()))); }
        @Override public void processDialogTerminated(DialogTerminatedEvent event) { evidence.event("sip.dialog.terminated", Map.of("dialog", String.valueOf(event.getDialog().getDialogId()))); }
        private String sipUri(String user, String host, int port) throws Exception {
            SipURI uri = addressFactory.createSipURI(user, host); uri.setPort(port); uri.setTransportParam(ListeningPoint.UDP); return uri.toString();
        }
        private static String randomTag() { return UUID.randomUUID().toString().replace("-", "").substring(0, 12); }
        @Override public void close() { try { stack.stop(); } catch (Exception e) { evidence.event("sip.stop.error", Map.of("error", e.toString())); } }
    }

    static String offerSdp(Config config) { return rttSdp(config.callerSipIp(), config.mediaSourcePort(), "sendonly"); }
    static String answerSdp(Config config) { return rttSdp(config.mediaTargetIp(), config.mediaTargetPort(), "recvonly"); }
    private static String rttSdp(String ip, int port, String direction) {
        return "v=0\r\n" + "o=baudot 0 0 IN IP4 " + ip + "\r\n" + "s=Baudot RTT probe\r\n"
                ...
                + "a=fmtp:" + RED_PAYLOAD_TYPE + " " + T140_PAYLOAD_TYPE + "/" + T140_PAYLOAD_TYPE + "/" + T140_PAYLOAD_TYPE + "\r\n"
